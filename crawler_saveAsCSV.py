...
# write as cvs
with open("street.csv","w",encoding='utf-8',newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["교차로명","주소"])
...
while True:
    ...
    for item in cur_items:
        ...
        with open('./street.txt','a',encoding='utf-8') as f:
            ...
            f.write("-------------------------------------------------\n")
        with open("street.csv","a",encoding='utf-8',newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([street_name,street_addr])
    i += cnt
